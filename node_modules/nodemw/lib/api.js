		...
		doRequest = function ( params, callback, method, done ) {
			...
			request( options, function ( error, response, body ) {
				...
				if ( error ) {
					...
					callback(`Request to API failed.` );

					if (error.errno === 'ETIMEDOUT') { console.log('Error (connection): something wrong with the connection.') }
					else if (error.hasOwnProperty('host')) console.log('Error (connection/address): the address', error.host, 'can not be reached.');
